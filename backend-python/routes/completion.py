...
class ChatCompletionBody(ModelConfigBody):
    ...
    session_id: str  # 添加 session_id 字段
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "system_name": None,
                "presystem": True,
                "session_id": "session-id",  # 在示例中包含 session_id
...
async def chat_completions(body: ChatCompletionBody, request: Request):
    ...
    # 从 Body 获取 session_id 和 isTeacher
    ...
    if (
        ...
        return await chat(model, body, request, completion_text, session_id, is_teacher, course_id)
...
async def chat(
    model: TextRWKV, body: ChatCompletionBody, request: Request, completion_text: str, session_id: str, is_teacher: bool, course_id: Union[str, None]
):
    # 根据is_teacher属性调整响应逻辑
    if is_teacher:
        # 教师模式：可以提供更详细的解释和教学指导
        print(f"教师模式：用户 {session_id} 正在使用教师权限")
